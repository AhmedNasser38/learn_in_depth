...
typedef struct {
	...
	EXTI_GPIO_Mapping_t  	EXTI_PIN; 		// Specifies the external interrupt GPIO mapping
											// Values Must be set according to @ref EXTI_Define

	uint8_t 				Trigger_Case;	// Specifies the external interrupt Trigger
											// Values Must be set according to @ref EXTI_Trigger_Define

	uint8_t					IRQ_EN;			// Specifies enable or disable EXTI (that enable the IRQ Mask in EXTI and also on NVIC Interrupt)
											// Values Must be set according to @ref EXTI_IRQ_Define
